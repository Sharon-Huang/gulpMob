...
gulp.task('browser-proxy', function() {
    var middleware = proxyMiddleware('/api',{
        target: 'http://dev.txgg.zonst.org', // 目标服务器
        changeOrigin: true
    });
    connect.server({
        root: './',
...
// 代理
gulp.task('proxy', gulpsync.sync(['sass', 'sass:watch', 'browser-proxy','browser-sync']));
// 可以通过对应的localhost:8080/api 打开代理的dev.txgg.zonst.org/api,但是请求接口的时候存在问题？？？
